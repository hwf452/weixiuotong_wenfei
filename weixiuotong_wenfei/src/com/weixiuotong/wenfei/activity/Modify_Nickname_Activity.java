...
public class Modify_Nickname_Activity extends Activity implements
		OnClickListener {
	...
	private void Modify_NickName() {
		...
		Parameter parameter = new Parameter("nickName", et1.getText()
				.toString());
		params.add(parameter);
		
		
		
		Parameter parameter4=new Parameter("address",Address);
		params.add(parameter4);
		int uid = sharedPreferences.getInt("UID", 0000000000);
		if (uid != 0000000000) {
			String UID = String.valueOf(uid);
			Parameter parameter5 = new Parameter("UID", UID);
